...
BINARY_Thresh = 140 # Sets the value for binary thresholding
...
distance = 10 # Distance (in pixels) at which search is being conducted        
...
while(cap.isOpened()):  
    ...
    if ret:
        ...
        2. The grayscale video is converted to a binary (black/white) video using the most simple algorithm
        You may want to add here some more sophisticated algorithms (e.g. adaptive thresholding) and also filter algorithms 
        to filter the "noise".
        '''
        ...
        #binary = cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY,11,5)
        ...
        '''
        This is the second step where more magic happens. 
        1. The binary video is being converted into a single line with the functions 'linesegmentation' and 'findnextpoint' 
